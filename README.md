...
$$S_h = \text{softmax}\left( \frac{Q_hK_h^T}{\sqrt H} \right), ~h = 1\dots\text{num\_heads}$$
$$Y_h = S_hV_h$$
$$Z = YW_o$$

Here 
* $X \in \mathbb{R}^{L \times F}$ &mdash; the input matrix; $L$ is sequence length (or set size), $F$ is number of features
* $W_q, W_k, W_v, W_o \in \mathbb{R}^{F \times F}$ &mdash; query, keys, values and output weight matrices
* $Q_h, K_h, V_h, Y_h \in \mathbb{R}^{L \times H}$ &mdash; submatrices of $Q, K, V, Y$, so-called heads; $H$ is number of features in the single head, $H \times \text{num\_heads} = F$. 
...
$$Y_{ij} = \left\{\text{softmax}(QK^T)V\right\}_{ij} = \sum_{l = 1}^L \left( \frac{\exp (\hat{S}_{il})}{\sum_{k = 1}^L\exp(\hat{S}_{ik})}\right) V_{lj} $$
here $\hat{S} = \frac{QK^T}{\sqrt H}$, the unnormalized scores (before softmax). We can rewrite this formula as follows:
$$Y_{ij} =  \frac{\sum_{l = 1}^L \exp (\hat{S}_{il}) V_{lj}}{\sum_{l = 1}^L\exp(\hat{S}_{il})} $$
...
while $s_{\max}$ can be big enough to its exponent would owerflow a half precision float, we can still hold its value without calculating the exponent directly. On the other hand $s_i - s_{\max} \leqslant 0, \forall i = 1\dots N$ , so $\exp(s_i - s_{\max}) \in (0, 1]$, and we can calculate $s_{\text{sum}} = \sum_{i=1}^N\exp(s_i - s_{\max})$ directly and store it. So sum of exponentials can be represented as a pair of numbers $\left(s_{\max}; s_{\text{sum}} \right),$ and recovered by calculating 
$$\sum_{i=1}^N \exp(s_i) = \exp(s_{\max}) \cdot s_{\text{sum}}$$

We can also represent a numerator in that manner, if we select $s_{\max} = \max_{i=1\dots N}s_i$ and $s_{\text{sum}} = \sum_{i=1}^N \exp(s_i - s_{\max})v_i$ then
$$
\sum_{i=1}^N \exp(s_i)v_i = 
\exp(s_{\max})\sum_{i=1}^N \exp(s_i - s_{\max})v_i =
\exp(s_{\max}) \cdot s_{\text{sum}} 
$$
...
$\sum_{i = 1}^N \exp(a_i) + \sum_{j=1}^M \exp(b_j)$ as the pair of numbers $(c_{\max}, c_{\text{sum}})$, where
...
$$Y_{ij} =  \frac{\cancel{\exp(\max_l (\hat{S}_{il}))} \sum_{l = 1}^L \exp (\hat{S}_{il} - \max_l (\hat{S}_{il})) V_{lj}}{\cancel{\exp(\max_l (\hat{S}_{il}))}\sum_{l = 1}^L\exp(\hat{S}_{il} - \max_l (\hat{S}_{il}))} $$
...
[^1]: For example in facebook's llama-7B $L$ is usualy equal to $2048$ and $\text{num\_heads} = 40$ and this is a smallest model in the family.

[^2]: Ususaly head dim $H$ is equal to $64$ or $128$. 
